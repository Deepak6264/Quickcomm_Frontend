...
const AccordionSummary = styled((props) => (
  <MuiAccordionSummary
    expandIcon={
      <ExpandMoreIcon sx={{ fontSize: "1.5rem", color: "#0652DD" }} />
    }
    {...props}
  />
))(({ theme }) => ({
...
export default function ShowCategory({ data, scid }) {
  ...
  const fetchAllBrands = async (subcategoryid) => {
    var result = await postData(
      "userinterface/user_get_all_brand_by_subcategoryid",
  ...
  const showAllSubCategory = () => {
    return data.map((item) => {
      return (
        <div>
          <Accordion
            expanded={expanded === item.subcategoryid}
            onChange={handleChange(item.subcategoryid)}
          ...
          <div>
            <Box
              sx={{
                ...
                display: "flex",
                flexWrap: "wrap",
                "& > :not(style)": { m: 1, width: 270, height: 500 },
              }}
            >
              <section
                elevation={3}
